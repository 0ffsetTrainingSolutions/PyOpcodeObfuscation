...
class PythonRemap:
    ...
    def __init__(self, python_dir):
        ...
        self.remapped_additional_mnemonic_dict = {}
    ...
    @staticmethod
    def remap_opcodes(mnemonic_dict, interpreter_mnemonic_dict):

        for mnemonic in mnemonic_dict:
            mnemonic_new_value = interpreter_mnemonic_dict[mnemonic]
            mnemonic_dict[mnemonic] = mnemonic_new_value
...
def main():
    ...
    python_mapping = sys.argv[2]

    with open(python_mapping) as f:
        data = f.read()
        data = data.replace("'", "\"")
        opcode_data = json.loads(data)
    ...
    with open(pyRemap.opcode_h, "r") as f:
        ...
        pyRemap.remapped_non_arg_mnemonic_dict = pyRemap.remap_opcodes(pyRemap.non_arg_mnemonic_dict, opcode_data)
        pyRemap.remapped_arg_mnemonic_dict = pyRemap.remap_opcodes(pyRemap.arg_mnemonic_dict, opcode_data)
        #pyRemap.remapped_additional_mnemonic_dict = pyRemap.remap_opcodes(pyRemap.additional_mnemonic_dict, opcode_data)
        
        pyRemap.remapped_additional_mnemonic_dict = pyRemap.additional_mnemonic_dict
        ...
        #opcode_h_data = pyRemap.replace_opcode_h_information(opcode_h_data, 
        #                                                     pyRemap.remapped_additional_mnemonic_dict)
